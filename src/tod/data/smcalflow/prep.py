...
class OnmtTextDatum:
    ...
    @classmethod
    def create_output_files(self, outbase: str) -> Dict[str, TextIO]:
        return {
...
def stringify_turn(
        ...
        tokenize_utterance: bool,) -> str:
    """Render a turn as a string and inserts corresponding delimiters before
    each segment in the string."""
    segments: List[str] = []
    ...
    if include_described_entities and turn.agent_utterance.described_entities:
        # NOTE: Should check if `described_entities` is empty or not.
        # Otherwise, this will introduce an empty token. This is fine for
        # OpenNMT though because it uses `string.split()` to tokenize the
        # string. See the `tokenizer` attribute in `torchtext.data.field.Field`
...
def create_onmt_text_datum_for_turn(
        ...
        include_program: bool,
        include_agent_utterance: bool,
        include_described_entities: bool,) -> OnmtTextDatum:
...
def create_context_turns(
    ...
    return [
        turn_lookup[tt]
        for tt in range(
            max(min_turn_index, curr_turn_index -
                num_context_turns), curr_turn_index
        )
    ]
...
def create_onmt_text_data_for_dialogue(
        ...
        include_program: bool,
        include_agent_utterance: bool,
        include_described_entities: bool,) -> Iterator[OnmtTextDatum]:
...
def prep_dialogue(
        ...
        include_agent_utterance: bool,
        include_described_entities: bool,
        onmt_text_data_outbase: str,) -> None:
