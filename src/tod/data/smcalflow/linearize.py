...
--
Tools for linearizing a program so that it's easier to predict with a seq2seq
model.
"""
import re
from typing import List, Tuple

from .lispress import (
    LEFT_PAREN,
    RIGHT_PAREN,
    Lispress,
    lispress_to_program,
...
from .program_utils import Idx, OpType
...
def to_canonical_form(tokenized_lispress: str) -> str:
    ...
    The canonical form is un-tokenized and compact; it also sorts named
...
def sexp_formatter(sexp_tokens: List[str]) -> List[str]:
    ...
    Inserts spaces after/before the leading/ending double quotes for value
    s-expressions within the s-expression.
    """
    sexp_str = " ".join(sexp_tokens)
...
def sexp_to_seq(s: Sexp) -> List[str]:
    if isinstance(s, list):
        return [LEFT_PAREN] + [y for x in s for y in sexp_to_seq(x)] +\
            [RIGHT_PAREN]
    else:
        return [s]
