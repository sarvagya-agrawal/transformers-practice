...
from .program_utils import DataflowFn, Idx, OpType, get_named_args
from .program_utils import is_idx_str as is_express_idx_str
...
from .sexp import LEFT_PAREN, RIGHT_PAREN, Sexp, parse_sexp, sexp_to_str
...
def render_pretty(lispress: Lispress, max_width: int = 60) -> str:
    ...
    Right inverse of `parse_lispress`
        (I.e. `parse_lispress(render_pretty(p)) == p`).
    E.g.:
    >>> lispress = ['describe', [':start', ['findNextEvent',
...
def parse_lispress(s: str) -> Lispress:
    ...
    '            #(PersonName "Elaine")))))))'
    >>> parse_lispress(s)
    ['describe', [':start', ['findNextEvent', ['Constraint[Event]',
        ':attendees', ['attendeeListHasRecipientConstraint',
    ...
    if isinstance(sexp, str):
        ...
        while i < len(sexp):
            ...
            if s == VALUE_CHAR and i + 1 < len(sexp):
                ...
                i += 1
        # special-case top-level values because we can't strip out the last
        # level of parens in the Sexp:
        if (
            isinstance(result, list)
            # value has been turned into a single str already here by
            # _render_value_expressions
            and len(result) == 1
...
def _render_lines(sexp: Lispress, max_width: int) -> List[str]:
    ...
    if isinstance(sexp, str) or len(sexp) <= 1 or len(compact) <= max_width:
        ...
        arg_lines = _group_named_args(
            [line for arg in args for line in _render_lines(
                arg, max_width=max_width)]
        )
...
def _is_idx_str(s: str) -> bool:
    return len(s) > 1 and s.startswith(VAR_PREFIX) and \
        s[len(VAR_PREFIX):].isnumeric()
...
def _named_arg_to_key(name: str) -> str:
    """Strips an initial `KEYWORD_PREFIX` off of `name`.
    Left-inverse of `key_to_named_arg`."""
...
def _key_to_named_arg(key: str) -> str:
    """Adds `KEYWORD_PREFIX` to the start of `key`.
    Right-inverse of `named_arg_to_key`."""
    return f"{NAMED_ARG_PREFIX}{key}"


def op_to_lispress(op: Op) -> Lispress:
    """
    Converts `op` to lispress in a way that can be undone
    (see `seq_to_program`).
    Note that for struct and call ops, op just has the name of the op
    (doesn't include args).
    """
    if isinstance(op, BuildStructOp):
        ...
        underlying_json_str = " ".join(
            json.dumps(underlying, separators=(
                " ,", " : "), indent=0).split("\n")
        )
...
def _program_to_unsugared_lispress(program: Program) -> Lispress:
    ...
    for expression in program.expressions:
        ...
        if isinstance(expression.op, (BuildStructOp, CallLikeOp)):
            curr = [op_lispress]
            named_args = sorted(get_named_args(expression)
                                )  # sort alphabetically
